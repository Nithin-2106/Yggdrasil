...
const TYPE_COLOR  = { Manhwa: '#A78BFA', Manga: '#FCD34D', Manhua: '#F43F5E' }
const STATUS_CONFIG = {
...
function SkeletonBlock({ w = '100%', h = '16px', style: extra = {} }) {
  return (
    <div style={{
      width: w, height: h,
...
function AddToListModal({ mangaData, existingEntry, isCompact, onClose, onSaved, onDeleted }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (existingEntry?._id) await axios.put(`${API}/${existingEntry._id}`, payload)
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              }}>{existingEntry ? 'Edit Entry' : 'Add to My List'}</span>
            </div>
            <button
              onClick={onClose}
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        color: hov ? C.text : C.textMuted, transition: 'color 0.2s', lineHeight: 1.3,
        overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical',
      }}>{char?.name?.full}</div>
      <div style={{ fontSize: '10px', color: C.textDim, marginTop: '3px', letterSpacing: '0.05em' }}>{char?.name?.native}</div>
    </div>
  )
}
...
function RelationCard({ edge }) {
  ...
  const relColor = {
    PREQUEL: C.primary, SEQUEL: C.crimson, ADAPTATION: C.gold,
...
export default function InfoPage({ anilistId }) {
  ...
  const fetchExisting = useCallback(async (mangaId) => {
    try {
      const r = await axios.get(API)
      const found = r.data.find(e => e.anilistId === mangaId)
  ...
  if (!data) return (
    <div style={{ color: C.textDim, fontFamily: '"Cinzel", serif', fontSize: '13px', padding: '40px 0' }}>
    ...
    <div style={{ animation: 'fadeIn 0.4s ease' }}>
            ...
            <div style={{ display: 'flex', gap: isCompact ? '20px' : '36px', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: isCompact ? 'center' : 'flex-start' }}>
              {aniScore && parseFloat(aniScore) > 0 && (
                <div>
                  ...
                  {data.favourites > 0 && (
                    <div style={{ fontSize: '10px', color: C.textDim, marginTop: '3px', letterSpacing: '0.05em' }}>{data.favourites.toLocaleString()} favourites</div>
                  )}
                </div>
              )}
              <div>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {readLinks.map((link, i) => (
                  <a key={i} href={link.url} target="_blank" rel="noopener noreferrer" style={{
                    fontSize: '11px', letterSpacing: '0.1em', color: C.primary,
                    padding: '6px 14px', border: `1px solid ${C.primary}44`,
                    background: C.primarySoft, fontFamily: '"Cinzel", serif',
      ...
      {charEdges.length > 0 && (
        <div style={{ marginBottom: '56px' }}>
          ...
          {charEdges.length > 12 && (
            <button onClick={() => setShowAllChars(s => !s)} style={{
